...
class EndToEndContract(unittest.TestCase):
    ...
    def test_real_dispatcher_command_reaches_mpv(self):
        with tempfile.TemporaryDirectory() as value:
            ...
            args=arglog.read_text().splitlines();self.assertIn("--aid=2",args);self.assertIn("--sid=1",args);self.assertTrue(any(value.startswith("--osd-playing-msg=OPENHTPC • CINÉMA AUTO → PURE") for value in args))
