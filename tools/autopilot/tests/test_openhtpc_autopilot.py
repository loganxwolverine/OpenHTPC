...
class CanonicalProjectState(unittest.TestCase):
 ...
 def test_57_software_cutover_is_complete(self):
  self.assertEqual((self.state["next_action"],self.state["next_action_kind"],self.state["software_implementation_allowed"]),("AWAIT_RC3_PREPARATION_AUTHORIZATION","RELEASE_PREPARATION",False))
 ...
 def test_69_rc1_and_rc2_are_no_go_and_rc3_is_unprepared(self):
  ...
  self.assertEqual(candidate["version"],"1.2.0-rc3");self.assertFalse(candidate["artifact_prepared"])
