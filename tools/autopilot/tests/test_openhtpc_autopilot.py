...
class CanonicalProjectState(unittest.TestCase):
 ...
 def test_57_software_cutover_is_complete(self):
  self.assertEqual((self.state["next_action"],self.state["next_action_kind"],self.state["software_implementation_allowed"]),("RC3_PHYSICAL_VALIDATION","PHYSICAL",False))
 ...
 def test_69_rc1_and_rc2_are_no_go_and_rc3_is_pending(self):
  self.assertEqual(self.state["release_candidate"]["physical_qualification"],"NO_GO");self.assertEqual(self.state["rc2_candidate"]["physical_qualification"],"PHYSICAL_GATE_NO_GO");candidate=self.state["next_candidate"]
  self.assertEqual(candidate["version"],"1.2.0-rc3");self.assertTrue(candidate["artifact_prepared"]);self.assertEqual(candidate["physical_qualification"],"PENDING_PHYSICAL_VALIDATION")
