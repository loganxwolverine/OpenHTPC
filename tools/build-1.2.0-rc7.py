#!/usr/bin/env python3
"""Build the deterministic OPENHTPC 1.2.0 RC7 physical-validation candidate."""
...
def build():
 ...
 common={"schema":1,"version":metadata["top_version"],"commit":commit,"artifact":archive.name,"sha256":sha,"physical_qualification": "PENDING","media_audio_fix": "HD_BITSTREAM_DISPLAY_RESYNC_LIFECYCLE"}
 validation.write_text(json.dumps({**common,"build":BUILD,"installation_method":"update.sh","human_physical_validation_required":True,"decisive_test":"MEDIA_BITSTREAM_DENON_AVR"},indent=2,sort_keys=True)+"\n",encoding="utf-8")
 report.write_text(json.dumps({**common,"build_id":BUILD,"focused_security_tests":"183/183 PASS","complete_regression":"815/815 PASS","rc1":"NO_GO","rc2":"PHYSICAL_GATE_NO_GO","rc3":"PHYSICAL_GATE_NO_GO","rc4":"PHYSICAL_BITSTREAM_PASS_DIAG_FALSE_NEGATIVE","status":"OPENHTPC_1_2_0_RC7_READY_FOR_PHYSICAL_VALIDATION"},indent=2,sort_keys=True)+"\n",encoding="utf-8")
