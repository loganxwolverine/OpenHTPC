#!/usr/bin/env python3
"""Build the deterministic OPENHTPC 1.2.0 RC9 qualified release candidate.
...
NAME = "OpenHTPC-1.2.0-RC9"
BUILD = "public-release-1.2.0-rc9"
ARTIFACTS = ROOT / "artifacts"
DEV_TRANCHE = "PUBLIC_RC9"
WORKSTREAM = "MEDIA_UX"
...
def load_devctl():
    ...
    if spec is None:
        raise RuntimeError("RC9_DEVCTL_SPEC_UNAVAILABLE")
...
def exact_head() -> str:
    ...
    if status.strip():
        raise RuntimeError("RC9_RELEASE_WORKTREE_DIRTY")
    commit = subprocess.run(
        ["git", "rev-parse", "HEAD"], cwd=ROOT, text=True,
        capture_output=True, check=True,
    ).stdout.strip()
    if not devctl.COMMIT_PATTERN.fullmatch(commit):
        raise RuntimeError("RC9_RELEASE_COMMIT_INVALID")
    return commit
...
def stage_exact_release(commit: str, staging: pathlib.Path, scratch: pathlib.Path) -> dict[str, Any]:
    ...
    if not devctl.COMMIT_PATTERN.fullmatch(upstream):
        raise RuntimeError("RC9_FLEX_UPSTREAM_INVALID")
    ...
    if digest(staged_binary) != digest(built):
        raise RuntimeError("RC9_FLEX_STAGE_MISMATCH")
    ...
    for marker in ("LiveActivityState", "FallbackFont"):
        if marker not in strings:
            raise RuntimeError(f"RC9_FLEX_REQUIRED_MARKER_MISSING:{marker}")
    ...
    for relative, markers in required_text_markers.items():
        ...
        for marker in markers:
            if marker not in text:
                raise RuntimeError(f"RC9_UX_REQUIRED_MARKER_MISSING:{relative}:{marker}")
...
def build():
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="openhtpc-rc9-release-") as raw:
            ...
            json.dumps(
                {
                    **common,
                    ...
                    "status": "OPENHTPC_1_2_0_RC9_QUALIFIED_FOR_PRERELEASE",
                },
                indent=2,
                sort_keys=True,
            ) + "\n",
        ...
        if errors:
            raise RuntimeError("RC9_RELEASE_VERIFY_FAILED: " + "; ".join(errors))
    except Exception:
        for path in (archive, checksum, validation, report):
            path.unlink(missing_ok=True)
        raise
