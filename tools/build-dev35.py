#!/usr/bin/env python3
"""Build the distinct OPENHTPC dev35 playback UX validation candidate.
...
NAME = "OpenHTPC-1.1-RC3-Candidate-Dev35"
...
def build() -> tuple[pathlib.Path, pathlib.Path, pathlib.Path]:
    ...
    if any(path.exists() for path in (archive, checksum, report)):
        raise FileExistsError("DEV35_ARTIFACT_COLLISION")
    ...
    report.write_text(json.dumps({
        "schema": 1, "product": "OPENHTPC", "version": "1.1.0-dev35",
        "build_id": "rc3-playback-ux-finalize-dev1", "artifact": archive.name,
        "sha256": sha, "baseline_version": "1.1.0-dev34",
        "baseline_commit": "fcd94395edf70e6bf37fdba1443ae05f16d12ca3",
        "status": "DEV35_TECHNICAL_TESTS_PASS",
