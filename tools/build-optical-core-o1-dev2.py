#!/usr/bin/env python3
"""Build the O1 Dev2 canonical-UI-truth physical-validation artifact."""
from __future__ import annotations
import gzip,hashlib,json,os,pathlib,tarfile,tempfile
import openhtpc_release_metadata as release_metadata

ROOT=pathlib.Path(__file__).resolve().parents[1]
NAME="OpenHTPC-1.1.3-Optical-Core-O1-Dev2"; BUILD_ID="optical-media-ui-truth-dev2"
BASELINE="23d4e2091c023f51fe54d81d5c7eb036d5d9fb23"; ARTIFACTS=ROOT/"artifacts"
...
def build():
 ...
 if any(path.exists() for path in (archive,checksum,report)): raise FileExistsError("OPTICAL_CORE_O1_DEV2_ARTIFACT_COLLISION")
 ...
 release_metadata.validate_archive(archive,BUILD_ID); sha=digest(archive)
 checksum.write_text(f"{sha}  {archive.name}\n",encoding="utf-8")
 report.write_text(json.dumps({"schema":1,"product":"OPENHTPC","version":metadata["top_version"],"build_id":BUILD_ID,"baseline":BASELINE,"artifact":archive.name,"sha256":sha,"dedicated_tests":"10/10 PASS","full_regression":"171/171 PASS","status":"OPTICAL_CORE_O1_DEV2_READY_FOR_PHYSICAL_VALIDATION","published":False},indent=2,sort_keys=True)+"\n",encoding="utf-8")
