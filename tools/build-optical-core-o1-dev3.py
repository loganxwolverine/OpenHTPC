#!/usr/bin/env python3
"""Build O1 Dev3 plus its machine-readable validator manifest."""
...
ROOT=pathlib.Path(__file__).resolve().parents[1];NAME="OpenHTPC-1.1.3-Optical-Core-O1-Dev3";BUILD="optical-state-lifecycle-validator-dev3";BASELINE="050546def871242b5a2781785e0d31d2651b42cb";ARTIFACTS=ROOT/"artifacts";EXCLUDED={".git","artifacts","__pycache__"}
def digest(path):
...
def build():
 ...
 if any(path.exists() for path in (archive,checksum,report,validation)):raise FileExistsError("OPTICAL_CORE_O1_DEV3_ARTIFACT_COLLISION")
 ...
 validation_data={"schema":1,"version":metadata["top_version"],"build":BUILD,"commit":commit,"artifact":archive.name,"sha256":sha,"intended_validator_phase":"OPTICAL_CORE_O1_DEV3","installation_method":"update.sh","stop_openhtpc":True,"required_pre_checks":[["/usr/bin/test","-x","{HOME}/.local/bin/openhtpc"]],"required_post_checks":[["{HOME}/.local/bin/openhtpc","version"],["{HOME}/.local/bin/openhtpc","doctor"],["{SOURCE}/tools/openhtpc-optical-forensic.py"]],"playback_allowed":False,"local_kde_observation_required":True,"observer":"optical","initializing_timeout_seconds":15,"purge_authorized":False}
 validation.write_text(json.dumps(validation_data,indent=2,sort_keys=True)+"\n",encoding="utf-8")
 report.write_text(json.dumps({"schema":1,"version":metadata["top_version"],"build_id":BUILD,"commit":commit,"baseline":BASELINE,"artifact":archive.name,"sha256":sha,"validation_manifest":validation.name,"dedicated_tests":"9/9 PASS","full_regression":"180/180 PASS","status":"OPTICAL_CORE_O1_DEV3_READY_FOR_ONE_COMMAND_PHYSICAL_VALIDATION","published":False},indent=2,sort_keys=True)+"\n",encoding="utf-8")
