#!/usr/bin/env python3
"""Build bounded OpenHTPC 1.1.3 Dev6 plus validator manifest."""
...
ROOT=pathlib.Path(__file__).resolve().parents[1];NAME="OpenHTPC-1.1.3-Optical-TMDb-Selection-UI-Dev6";BUILD="optical-tmdb-selection-ui-dev6";BASELINE="642d7497cd541f9273d7272077f8b35c84929923";ARTIFACTS=ROOT/"artifacts";EXCLUDED={".git","artifacts","__pycache__","diagnostics"}
def digest(path):
...
def build():
 ...
 if any(path.exists() for path in (archive,checksum,report,validation)):raise FileExistsError("OPTICAL_TMDB_SELECTION_UI_DEV6_ARTIFACT_COLLISION")
 ...
 validation_data={"schema":1,"version":metadata["top_version"],"build":BUILD,"commit":commit,"artifact":archive.name,"sha256":sha,"intended_validator_phase":"OPTICAL_TMDB_SELECTION_UI_DEV6","installation_method":"update.sh","stop_openhtpc":True,"required_pre_checks":[["/usr/bin/test","-x","{HOME}/.local/bin/openhtpc"]],"required_post_checks":[["{HOME}/.local/bin/openhtpc","version"],["{HOME}/.local/bin/openhtpc","doctor"]],"playback_allowed":False,"local_kde_observation_required":True,"observer":"optical","presentation_convergence_timeout_seconds":5,"purge_authorized":False}
 validation.write_text(json.dumps(validation_data,indent=2,sort_keys=True)+"\n",encoding="utf-8")
 report.write_text(json.dumps({"schema":1,"version":metadata["top_version"],"build_id":BUILD,"commit":commit,"baseline":BASELINE,"artifact":archive.name,"sha256":sha,"validation_manifest":validation.name,"focused_tests":"44/44 PASS","full_regression":"205/205 PASS","status":"OPTICAL_TMDB_UI_DEV6_READY_FOR_PHYSICAL_VALIDATION","published":False},indent=2,sort_keys=True)+"\n",encoding="utf-8")
