...
check("candidate_version", metadata.get("version") == "1.1.0-dev34", str(metadata.get("version")))
check("candidate_build", metadata.get("build_id") == "rc3-playback-ui-persistence-osd-fix-dev1", str(metadata.get("build_id")))
...
check("no_unreleased_later_dev", not re.search(r"1\.1\.0-dev(?:3[5-9]|[4-9][0-9])\b", combined, re.I))
