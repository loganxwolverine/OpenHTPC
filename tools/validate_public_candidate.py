...
check("candidate_version", metadata.get("version") == "1.1.0-dev38", str(metadata.get("version")))
check("candidate_build", metadata.get("build_id") == "rc3-final-dvd-policy-refresh-corrective-dev1", str(metadata.get("build_id")))
...
check("ui_asset_count", len(ui) == 37, str(len(ui)))
for path in ui:
...
check("no_unreleased_later_dev", not re.search(r"1\.1\.0-dev(?:39|[4-9][0-9])\b", combined, re.I))
