...
SDL_Texture *load_texture_from_file(const char *path)
{
    ...
    if (path != NULL) {
        ...
        if (length >= 4 && !strcasecmp(path + length - 4, ".svg"))
            return rasterize_svg_from_file(path, -1, -1, NULL);
...
SDL_Texture *rasterize_svg_from_file(const char *path, int w, int h, SDL_Rect *rect)
{
    ...
    if (length < 0 || length > 8 * 1024 * 1024 || fseek(stream, 0, SEEK_SET) != 0) { fclose(stream); return NULL; }
    char *buffer = malloc((size_t) length + 1);
    if (buffer == NULL) { fclose(stream); return NULL; }
