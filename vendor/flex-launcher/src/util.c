...
void parse_config_file(const char *config_file_path)
{
    if (config_file_path != NULL) {
        ...
        config.config_file_path = strdup(config_file_path);
    }
...
int config_handler(void *user, const char *section, const char *name, const char *value)
{
    ...
    if (MATCH(section, "General")) {
        ...
        else if (MATCH(name, "LiveOpticalState")) {
    ...
    // Parse menus/entries
        ...
        if (MATCH(name, "BackgroundImage")) {
            free(menu->background_path);
            menu->background_path = strdup(value);
        ...
        if (token != NULL) {
            ...
            entry->context_cmd = NULL;
            entry->context_title = NULL;
            entry->context_texture = NULL;
            entry->context_selected = false;
        }
        ...
        for (i = 0; i < 5 && token != NULL; i++) {
            if (i == 0)
                ...
                entry->context_cmd = strdup(token);
            else if (i == 4)
                entry->context_title = strdup(token);

            token = strtok(NULL, (i >= 3) ? "" : delimiter);
        }

        // Delete entry if parse failed to find at least 3 valid tokens
...
{
    if (entry == NULL || spaces <= 0)
        return entry;
    if (direction == DIRECTION_LEFT) {
        for (int i = 0; i < spaces && entry != NULL; i++)
